...
*/
	...
	public static CommonSettings getCommonSettings() {
		...
		String automationcontrolexcelsheetpath = System.getProperty("user.dir")+ConfigReader.getValue(automationcontrolexcelpath);
		
		try(FileInputStream fis =new FileInputStream(automationcontrolexcelsheetpath); 
				Workbook wb = WorkbookFactory.create(fis);) {
	...
	public static List<TestConfig> getSuitesToRun() {
		...
		try (FileInputStream fis=new FileInputStream(automationcontrolexcelsheetpath);Workbook wb=WorkbookFactory.create(fis);){
			...
			while (true) {
				if (sheet.getRow(startRowNum) == null || "".equalsIgnoreCase(sheet.getRow(startRowNum).getCell(columnToLookFlag).getStringCellValue()) )
					...
					if (sheet.getRow(startRowNum).getCell(columnToLookFlag).getStringCellValue().equalsIgnoreCase(Y)) {
